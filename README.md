...
## Usage
    ...
    $ git clone --depth=1 https://github.com/m666m/1key-tls-repository
    $ cd 1key-tls-repository
...
详细使用方法会在部署成功后列出，示例如下：
    ...
    证书SAN属性的域名: mypi.local
    项目名: 1key-tls-repository
    compose.yaml 已更新
    正在构建镜像并启动容器...
    ...
    推送测试：
      ...
      docker tag 1key-tls-repository-registry2:latest 192.168.1.100/    1key-tls-repository-registry2:latest

      docker push 192.168.1.100/1key-tls-repository-registry2:latest

    ---------- 分发证书 ----------

    根据当前项目名（目录名），推测卷名如下：

      证书卷名: 1key-tls-repository_certs
      认证卷名: 1key-tls-repository_auth
      数据卷名: 1key-tls-repository_data

    提取证书到当前目录：

      docker run --rm -v 1key-tls-repository_certs:/certs alpine cat /certs/domain.crt > domain.crt
    ...
    ---------- 日常使用 ----------
    启动仓库容器：
      cd /home/pi/1key-tls-repository && docker compose up
    停止仓库容器：
      cd /home/pi/1key-tls-repository && docker compose down

    ---------- 重置证书和密码 ----------

    如需重新生成证书（例如更换域名）或更改密码，但保留镜像数据：

      1. 停止容器：cd /home/pi/1key-tls-repository && docker compose down

      2. 删除卷中的证书和密码文件：

         docker run --rm -v 1key-tls-repository_certs:/certs alpine rm -f /certs/domain.crt /certs/domain.key

         docker run --rm -v 1key-tls-repository_auth:/auth alpine rm -f /auth/htpasswd
